...
const wordsAppearInOrder = (titleWords: string[], queryWords: string[]): boolean => {
    ...
    for (let i = 0; i < queryWords.length; i++) {
        ...
        while (titleIndex < titleWords.length) {
            if (wordSimilarity(queryWord as string, titleWords[titleIndex] as string) >= 0.7) {
                // Match found
                titleIndex++
                break
            }
...
const getAdjacentSequencePosition = (titleWords: string[], queryWords: string[]): number => {
    for (let i = 0; i <= titleWords.length - queryWords.length; i++) {
        let match = true
        for (let j = 0; j < queryWords.length; j++) {
            if (wordSimilarity(queryWords[j] as string, titleWords[i + j] as string) < 0.7) {
                match = false
                break
            }
        }
